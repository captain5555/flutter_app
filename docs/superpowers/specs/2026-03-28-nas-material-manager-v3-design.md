...
注：v3.0 暂不支持嵌套文件夹，仅支持固定分类（images/videos/others）

| 字段 | 类型 | 说明 |
|------|------|------|
| id | INTEGER | 主键 |
| user_id | INTEGER | 用户 ID |
| folder_type | VARCHAR(50) | 文件夹类型：images/videos/others |
...
以下功能计划在 v3.0 之后的版本中实现：

### v3.1 规划
- [ ] 素材搜索功能（按文件名、类型、日期范围搜索）
- [ ] 用户存储空间配额管理
- [ ] 大文件上传进度反馈（WebSocket/SSE）
- [ ] 嵌套文件夹支持
